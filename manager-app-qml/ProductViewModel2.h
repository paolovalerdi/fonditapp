...
public:
    ...
    explicit ProductViewModel2(QObject *parent = nullptr);
    ...
    ProductViewModelCallback2 *getCallback() const;
    void setCallback(ProductViewModelCallback2 *value);

private:
    ProductsDao productDao = ProductsDao(ProductionDatabase::getInstance());
    QList<Product> productList = productDao.getAllProducts();
    ProductViewModelCallback2 *callback2;
};
